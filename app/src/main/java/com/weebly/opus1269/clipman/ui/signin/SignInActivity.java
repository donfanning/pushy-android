...
public class SignInActivity extends BaseActivity implements
  ...
  private void handleSignInResult(@Nullable GoogleSignInResult result) {
    if (!mConnectionFailed && (result != null)) {
      mErrorMessage = "";
      if (result.isSuccess()) {
  ...
  class DevicesReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
      ...
      switch (action) {
        case Intents.TYPE_MY_DEVICE_REMOVED:
          // device remove message sent, now unregister
          setProgressMessage(getString(R.string.unregistering));
          doUnregister();
          break;
        case Intents.TYPE_MY_DEVICE_REGISTERED:
          // registered
          dismissProgress();
          break;
        case Intents.TYPE_MY_DEVICE_UNREGISTERED:
          // unregistered, now signout
          doSignOut();
          break;
        case Intents.TYPE_MY_DEVICE_REGISTER_ERROR:
          // registration error
